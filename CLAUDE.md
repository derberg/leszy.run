...
- `confirm_rssi_cdbm`: **NULL by default = disabled.** The **crossing bar**, and the second half of a two-tier gate. `rssi_threshold` answers "may this read be followed at all" and must stay permissive or weak tags are lost; this answers "did the tag actually reach the gate". **START** requires the accumulated *peak* to clear it; **FINISH** requires the individual read to clear it (first-read semantics are preserved — a rejected weak read is skipped, NOT added to `finishedParticipants`, so the real crossing still counts). `stopRace`'s pending flush applies the same bar. A single threshold cannot do both jobs: tag performance spans ~30 dB, so one value trades missed runners against phantom crossings. Measured on real races: at `-5000` only 8 of 20 runners were detected at the start line; at `-6500`, 19 of 20 — but three of those peaked at only −62…−64 dBm and were far-field loiterers, not crossings. Real gate passes peak at −32…−50 dBm, so `-5500` separates them. A start rejected by the bar falls through to gun-time backfill; a rejected finish leaves the runner on course for manual closing — both are visible failures, unlike a phantom crossing's silently wrong time.
