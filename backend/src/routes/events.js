...
export async function eventsRoutes(fastify) {
  ...
  fastify.patch('/events/:id', async (req, reply) => {
    const allowed = ['name', 'description', 'date', 'location', 'rfidMode', 'rfidTopicMain', 'rfidTopicFinish', 'rssiThreshold', 'declineThresholdCdbm', 'fallbackSeconds', 'gunBackfillSeconds', 'gunBackfillEnabled', 'minFinishSeconds', 'slug', 'publicResultsUrl', 'eventUrl', 'visibility']
    const updates = {}
