  ...
  fastify.get('/calendar-events/duplicates', async (request, reply) => {
    ...
    while (true) {
      const { data, error } = await supabase
        ...
        .select('id, name, date, location, voivodeship, source, source_id, registration_url, source_url, event_type, distances, distances_meters, organizer, price_from, price_to, description, lat, lng, is_night, is_charity')
        .eq('status', 'active')
