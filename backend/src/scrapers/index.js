...
const sources = [
  ...
  {
    name: 'maratonczykpomiarczasu',
    scrape: scrapeMaratonczykpomiarczasu,
    table: 'scraper_maratonczykpomiarczasu',
    mapRow: (raw) => ({
      name: raw.name,
      date: raw.date,
      location: raw.location || null,
  ...
  {
    name: 'zapisyvaldano',
    scrape: scrapeZapisyvaldano,
    table: 'scraper_zapisyvaldano',
    mapRow: (raw) => ({
      name: raw.name,
      date: raw.date,
      location: raw.location || null,
  ...
  {
    name: 'plustiming',
    scrape: scrapePlustiming,
    table: 'scraper_plustiming',
    mapRow: (raw) => ({
      name: raw.name,
      date: raw.date,
      location: raw.location || null,
  ...
  {
    name: 'biegnijmy',
    scrape: scrapeBiegnijmy,
    table: 'scraper_biegnijmy',
    mapRow: (raw) => ({
      name: raw.name,
      date: raw.date,
      location: raw.location || null,
...
async function mergeIntoScraperAll({ dryRun = false } = {}) {
  ...
  for (const source of sortedSources) {
    const stats = { source: source.name, total: 0, created: 0, updated: 0, skipped: 0, skippedReasons: { non_running: 0, past_date: 0, junk: 0, foreign: 0, virtual: 0 }, errors: [], createdNames: [], updatedNames: [], rows: [] }
    ...
    try {
      ...
      for (const raw of allRows) {
        try {
          ...
          // "Wtorki Biegowe - Maraton na Raty" recurring weekly run series (seen on
          ...
          // Races run virtually. See looksVirtual() for what counts as one and
