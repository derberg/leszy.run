...
// Uses local Claude CLI with web search to find event websites, regulamin links,
// distances, and types for events missing data in scraper_all.
//
// Targets maratonypolskie by default (lowest data quality), but works for any source.
//
// Options:
//   --source <name>   Filter by source (default: maratonypolskie)
...
const sourceFilter = args.includes('--source') ? args[args.indexOf('--source') + 1] : 'maratonypolskie'
...
Find the event's official website, registration page, and regulamin (regulations) PDF or page.
From the regulamin or event page, extract race distances and classify the event type.

Return ONLY valid JSON, no other text:
{
  "website": "https://example.pl" or null,
  "registration_url": "https://example.pl/zapisy" or null,
  "regulamin_url": "https://example.pl/regulamin" or null,
...
- If you cannot find the event at all, return all nulls and empty array
...
async function main() {
  ...
  while (true) {
    const { data, error } = await supabase
      .from('scraper_all')
      .select('id, name, date, location, distances, event_types, is_kids, website, regulamin_url, registration_url')
  ...
  // Filter to events missing any data
  const needsWork = allRows.filter(r => {
    ...
    const noRegulamin = !r.regulamin_url
    return noType || noDist || noWebsite || noRegulamin
  })
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    // Regulamin
    if (result.regulamin_url && !row.regulamin_url) {
      updates.regulamin_url = result.regulamin_url
    }
    ...
    for (const [k, v] of Object.entries(updates)) {
      ...
      console.log(`    ${dryRun ? 'WOULD' : '✓'} ${k}: ${old} → ${Array.isArray(v) ? v.join(', ') : v}`)
    }
