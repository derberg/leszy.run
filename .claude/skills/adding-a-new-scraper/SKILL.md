...
These two fields determine whether your scraper's rows merge with existing same-event rows from other scrapers. `hasDistinguishingConflict` compares the `style:` tags of two rows as a SET, and a mismatch in EITHER DIRECTION rejects the merge: `{trail}` against `{trail, nw}` is a conflict, and so is `{trail}` against `{ocr}`. One-sided absence is the exception it tolerates (except for `kids:`), so a row with no style tag at all still merges. Beware the asymmetry that follows: emitting MORE tags than the other sources is as damaging as emitting none, and a specialist source is the one most likely to do it. Measured 2026-09-17 on biegigorskie: tagging every mountain race `{trail, ultra}` blocked merges with generalist rows tagged `{trail}`, and dropping `ultra` then blocked merges with the rows tagged `{trail, ultra}`, giving 23 duplicates one way and 28 the other. Match what the other sources say about the same races, and verify it in the dry-run before you apply.
...
| Distinguishing-tag guard rejects legitimate merges | Dry-run shows `created=N` for events you know exist in scraper_all from another source | Your `style:` tag set differs from theirs in either direction. Print both sides with `distinguishingTags()` before guessing: also check the tags each NAME contributes, since a name carrying "Ultra" or "Trail" adds them whatever `event_types` says. Section 5b |
