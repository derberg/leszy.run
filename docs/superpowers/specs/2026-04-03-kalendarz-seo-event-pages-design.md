...
`GET https://<project>.supabase.co/functions/v1/sitemap`

### Behavior

1. Static entries: `/`, `/kalendarz`, `/kalendarz/dodaj`, `/events`
2. Query `calendar_events` where `status = 'active'` and `date >= today - 7 days`
3. Compute slug for each, emit `<url>` entry with `<loc>`, `<lastmod>`, `<changefreq>weekly</changefreq>`
4. Return XML with `Content-Type: application/xml` and `Cache-Control: public, max-age=3600`

### robots.txt update

Change sitemap URL to point to the edge function:
```
Sitemap: https://<project>.supabase.co/functions/v1/sitemap
```
